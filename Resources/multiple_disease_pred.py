...
def app():
    ...
    if (selected == 'Diabetes Prediction'):
        ...
        with col1:
            Skfloathickness = float(st.text_input('Skin Thickness value'))

        with col2:
            Insulin = int(st.text_input('Insulin Level'))
        ...
        if st.button('Diabetes Test Result'):
            diab_prediction = diabetes_model.predict([[Pregnancies, Glucose, BloodPressure, Skfloathickness, Insulin, BMI, DiabetesPedigreeFunction, Age]])

            if (diab_prediction[0] == 1):
              st.write(diab_prediction[0])
